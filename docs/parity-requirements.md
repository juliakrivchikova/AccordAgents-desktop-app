...
| **Removing a device stops it commanding a machine, but not reading it.** A machine meets every controller the owner trusts in its own room, sealed with one key that all of them hold. Taking a device off the roster makes the machine reject its commands and its acknowledgements immediately, and leaves it able to open traffic in that room and every retained event it already has. Revoking authority and revoking confidentiality are not the same act, and only the first happens. | 2 | `MachinePeerFabric.channelIdFor` meets non-machine peers in `options.home.rendezvousId`; `machineTrustStore.accept` removes the peer's authority without changing `relaySealKeyBase64`. Verified 2026-09-07 by reading the trust and fabric paths on `taylor/machine-installer`. | Open. Closing it needs a decision the User has not made: rotate the machine's room and hand the new key to each remaining device sealed to its own identity, or seal each device-to-device channel with a key derived from the two devices' enrolled keys so a shared room key never grants reading at all. Both change the pairing surface, so neither is taken silently. |
