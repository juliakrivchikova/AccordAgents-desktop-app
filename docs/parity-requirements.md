...
| --- | --- | --- | --- |
| **AWS setup can report ready while Claude is not signed in, and offers no browser sign-in for Claude.** The generic setup requires Codex authentication only; a Claude-specific setup emits instructions to run `claude auth login` manually on the worker instead of providing the login interaction. | 1 | Verified 2026-09-13 in this laptop's isolated cloud environment: native `claude auth status --json` returned `loggedIn: false`, `authMethod: none`, while Codex login status succeeded and the saved AWS setup result was `Worker ready (1 warning).` The path is `AwsWorkerSetupService` → `CloudRunDoctorService.requiredChecksForOptions` / `setup`. | Implementation in progress: generic AWS setup and Diagnostics require enabled Codex/Claude providers; Claude uses native `claude auth login` with browser authorization and a scoped code input. The real participant selector and Settings Diagnostics both show native browser sign-in; cancellation, retry, wrong-request rejection and restoring the form after reopening Settings are verified in isolated Electron against the real AWS worker; successful browser authorization remains unverified because automatic approval review repeatedly rejected the Google sign-in click despite manual approvals. Do not close this defect until that real end-to-end check passes. |
