...
- `app_permissions_request_change` is available to every chat participant. It can request portable grants (`workspaceWrite` or `webAccess`), command-specific `shellRules`, or Claude-only `providerNative.allowedTools` for the requesting participant only, creates a pending approval, and never grants permissions directly.
