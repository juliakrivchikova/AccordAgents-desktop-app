...
3. CLI detection seeds one generic saved participant preset per installed CLI provider unless the user deleted that provider's seed preset. Seeded presets use the `generic-participant` role and Codex/Claude/Gemini logo avatars.
...
5. If no participant with role `administrator` is present, `ChatService` injects `@assistant` using an installed CLI provider, preferring Codex CLI, then Claude Code, then Gemini CLI (Antigravity). Existing `@admin` chats remain mention-compatible and display as Chat Assistant.
