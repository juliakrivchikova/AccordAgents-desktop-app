...
- `app_chat_get_context`, `app_chat_get_participants`, `app_chat_get_participant_activity`, `app_chat_read_messages`, `app_chat_list_attachments`, and `app_chat_read_attachment` are read-only context tools available to every issued participant token. They let agents read the active turn, roster, authoritative roster/UI-consistent member activity, focused message pages, and visible image attachments without rereading full history files. The activity snapshot deliberately follows roster/UI state; unlike `app_chat_get_participant_request_status`, it does not unconditionally reconcile orphaned requests while unrelated live work prevents normal chat recovery.
