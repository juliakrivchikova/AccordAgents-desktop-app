...
- Antigravity native `/goal` currently uses its macOS interactive CLI through the system `/usr/bin/expect` PTY. The app waits for Antigravity's returned interactive prompt as the terminal boundary. Because the current TUI permission dialog has no stable machine-readable approval protocol, a goal that opens that dialog fails with an actionable error instead of hanging; Auto-review or pre-granted permissions avoid that dialog.
