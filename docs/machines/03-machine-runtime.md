...
| — | desktop stale-run sweep | a pending bubble of a member whose home is a machine is never swept as "Interrupted before completion" on the desktop: the machine owns every outcome of its runs, including runs it started itself (a resume after an approval) that the desktop never registered; the machine in turn ignores a swept copy of a bubble it already finished |
