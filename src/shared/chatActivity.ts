...
function pendingMessageItems(
  ...
  if (message.metadata?.pendingChoice?.status === "pending") {
    items.push({
      ...
      preview: message.metadata.pendingChoice.question || previewText(message.content) || "A participant is waiting for a choice.",
      createdAt: message.createdAt,
      updatedAt,
      participant,
      target
  ...
  if (pendingMentions.length > 0) {
    items.push({
      ...
      preview: pendingMentions.map((mention) => `@${mention.targetHandle}`).join(", "),
      createdAt: message.createdAt,
      updatedAt,
      participant,
      target
  ...
  if (message.metadata?.participantRequest?.status === "pending_approval") {
    items.push({
      ...
      preview: participantRequestPreview(message),
      createdAt: message.createdAt,
      updatedAt,
      participant,
      target
