...
export const USER_AVATAR: AvatarSpec = { kind: "user", label: "You" };
export const ARBITER_AVATAR: AvatarSpec = { kind: "arbiter", label: "Arbiter" };
...
export function AgentAvatarWithDetails(props: {
  ...
  async function copySessionId(event: MouseEvent<HTMLButtonElement>): Promise<void> {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="agent-avatar-details"
        side="bottom"
        align="start"
        sideOffset={8}
...
function avatarGraphic(spec: AvatarSpec): ReactNode {
  if (spec.kind === "user") {
    return (
  ...
  if (spec.kind === "arbiter") {
    return <img src={JUDGE_AVATAR_URL} alt="" aria-hidden="true" />;
  }
