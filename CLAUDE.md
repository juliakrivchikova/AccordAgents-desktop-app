...
- `command.ts` — `runCommand` helper used by every service that spawns a subprocess. Honors `AbortSignal`, enforces `timeoutMs` with SIGTERM→SIGKILL escalation. CLI runs use the configurable `cliAgentRunTimeoutMs` (`src/shared/cliAgentRunSettings.ts`): default 24h, min 1h, max 7d — an agent turn is not capped at minutes. Remote runs default to 24h independently (`remoteRuns.ts`). sqlite calls use 10s.
