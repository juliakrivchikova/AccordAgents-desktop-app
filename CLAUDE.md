...
- `settings.ts` — Reads/writes JSON settings under Electron `userData`. API keys are encrypted with Electron `safeStorage`; `hasApiKey` is the only key-related field exposed to the renderer. Owns `chatRoleConfigs` (instructions templates) and `chatParticipantConfigs` (handle/role/CLI bindings) used by `ChatService`.
